...
echo -e "  40+ Cloud-Dienste: Google Drive · Dropbox · OneDrive · S3 · SFTP · und mehr"
echo ""
echo "  ─────────────────────────────────────────"
echo ""
...
# GitHub API: neueste Version ermitteln (Cache-Control verhindert veraltete Antworten)
RELEASE_JSON=$(curl -fsSL \
    -H "Cache-Control: no-cache" \
    -H "Accept: application/vnd.github+json" \
    "https://api.github.com/repos/Zenovs/Nubix/releases/latest" 2>/dev/null || echo "")

LATEST=$(echo "$RELEASE_JSON" \
...
if [[ -n "${APPIMAGE_URL:-}" ]]; then
    ...
    curl -fL --progress-bar --retry 3 --retry-delay 3 "$APPIMAGE_URL" -o "$APPIMAGE_PATH"
